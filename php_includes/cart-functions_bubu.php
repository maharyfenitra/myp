...
function getCartContent()
{
	...
	while ($row = dbFetchAssoc($result)) {
		if ($row['pd_thumbnail']) {
			$row['pd_thumbnail'] = WEB_ROOT . 'images_small/product/' . $row['pd_thumbnail'];
		} else {
			$row['pd_thumbnail'] = WEB_ROOT . 'images_small/no-image-small.png';
		}
		$cartContent[] = $row;
	}
...
{
	if (!$cartId && isset($_GET['cid'])&&isValidField($_GET['cid']) && (int)$_GET['cid'] > 0) {
		$cartId = (int)$_GET['cid'];
	}
...
function deleteCart()^M
{^M
        $sid = session_id();^M
	$sql = "DELETE FROM tbl_cart
                        FROM tbl_cart ct^M
                        WHERE ct_session_id = '$sid'";^M
        $result = dbQuery($sql);^M
}^M
