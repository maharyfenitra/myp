...
//				WHERE user_name = '$userName' AND user_password = PASSWORD('$password')";
		...
		if (dbNumRows($result) == 1) {
			...
			// fetch the user credentials
	                ...
	                $sql = "SELECT user_level FROM tbl_user WHERE user_id = '{$row['user_id']}'";
       	         	$result = dbQuery($sql);

                	if (dbNumRows($result) == 1) {
                        	$row = dbFetchAssoc($result);
                        	$_SESSION['plaincart_user_level'] = $row['user_level'];
                	} else {
                        	$_SESSION['plaincart_user_level'] = 1;
                	}
			...
			$errorMessage = 'Wrong username or password: '.$userName.'/'.$password;
		}
...
function doLogout()
{
	if (isset($_SESSION['plaincart_user_id'])) {
		...
		unset($_SESSION['plaincart_user_level']);
		//session_unregister('plaincart_user_id');
		//session_unregister('plaincart_user_level');
	}
