...
//        if (strcmp($text,'') >= 0 )  {
                 $language_item_list_name .= "<tr><td><input type='hidden' name='name_lang[]' value='$lang' size='2'>$lang<input type='hidden' name='name_page[]' value='$page'><input type='hidden' name='name_item[]' value='$item' size='5'></td><td><textarea  rows='5'  name='name_text[]' cols='60'>$text</textarea><td>
      <input type='text' name='name_url[]' value='$url' size='5'></td><td><a href='javascript:deleteLanguageItem(\"".$lang."\",\"".$item."\",".$productId.")'>x</a></td></tr>";
...
?> 
<form class="addslach" action="processProduct.php?action=modifyProduct&productId=<?php echo $productId; ?>" method="post" enctype="multipart/form-data" name="frmAddProduct" id="frmAddProduct">
 <p align="center" class="formTitle">Modify Product</p>
 
 <table width="1000px" border="0" align="center" cellpadding="5" cellspacing="1" class="entryTable">
  <tr> 
   <td width="150" class="label" >Category</td>
   <td class="content" width="75%"> <select name="cboCategory" id="cboCategory" class="box">
...
?>	 
 ...
 <tr style="display:none"> 
  ...
  <?php
$productinfo->setPdid($_GET['productId']);
foreach($alltype as $type){
echo "<tr><td colspan=1 class='label'>".$type['label']."</td><td colspan=1 align='right' class='content'>".$productinfo->getPrice($admin->getIdTypeFor($type['label']))."</td></tr>";
   }
...
?>
    ...
    Image </a> <br><br> <?php echo PRODUCT_IMAGE_DIR . $pd_thumbnail; ?>
    <?php
	}
         else{
         	
         	?>
         	<input name="imageName" type="text" id="imageName" class="box" size="60" value="<?php echo $pd_thumbnail;?>">
...
<?php 
$pd_id=$_GET['productId'];
?>
<tr><td width="150" class="label">Add new image</td><td width="150" class="content">
<a href="javascript:PopupWindow(this,'index.php?view=addImage&&pd_id=<?php echo $pd_id; ?>');">Add Image</a>
</td></tr>
<script>

//"index.php?view=addImage&&pd_id=<?php echo $pd_id; ?>"
//"javascript:PopupWindow(this,'index.php?view=addImage&&pd_id=<?php echo $pd_id; ?>');"
function PopupWindow(source, strWindowToOpen){
var strWindowFeatures = "toolbar=no,resize=no,titlebar=no,";
strWindowFeatures = strWindowFeatures + "menubar=no,width=1013,height=899,maximize=null";
...
$f="/image_uploaded/".$rang['pd_image'];?>
      <tr width="150" class="label"><td>
      
      <?php $url=urlencode ($_SERVER['REQUEST_URI']);?>
      <a href="cycle.php?origine_url=<?php echo $url?>&delete_image=<?php echo $rang['pd_image']?>&pdid=<?php echo $_GET['productId'];?>">Delete</a></td><td>
 <img src =<?php echo '"'.$f. '"' .SetImageDimensions ($f,440,200)?> class="im"/>  
